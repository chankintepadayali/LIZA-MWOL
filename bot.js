...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅️Arjunser working ' + config.WORKTYPE + ' 𝚗𝚘𝚠 '));
          // thanks to afnanplk
...
*/
                        try {
                            ...
                            if (config.NOLOG == 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ 𝛢રյ𝛖𝛈𝑆𝜀ર🎈࿐ ~______~*' +
                                    '\n\n*👾 ' + error + '*\n\n```Report errors\njoin ⚠️Warning bot not allowed in the group\https://chat.whatsapp.com/J5Z3gyoxidd1JDiJzA9a9x``` ' 
                                    , MessageType.text);
                            }
                        }
                    }
